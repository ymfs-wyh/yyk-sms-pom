...
public class NotificationHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		logger.debug("连接成功...");
		// 这块会实现自己业务...
		clients.put(String.valueOf(session.getAttributes().get("uid")), session);
	...
	public void sendMessageToUser(String userId, TextMessage message) {
		for (WebSocketSession session : clients.values()) {
			if (userId.equals(String.valueOf(session.getAttributes().get("uid")))) {
				if (session.isOpen()) {
					try {
					...
					clients.remove(session.getAttributes().get("uid"));
				}
				break;
			}
		}
